...
def run_sanity_check(test_dir):
    ...
    print('Please enter the path to the file that contains your test cases for the \
    ...
    test_functions_for_get = list(filter(lambda x:
                                  inspect.getsource(getattr(module, x)).
                                  find('.get(') != -1,
                                  test_function_names)
                                  )
    test_functions_for_post = list(filter(lambda x:
                                   inspect.getsource(getattr(module, x)).
                                   find('.post(') != -1,
                                   test_function_names)
                                   )
